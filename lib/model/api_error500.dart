...
class ApiError500 {
  ...
  static ApiError500 fromJson(dynamic value) {
    if (value is Map) {
  ...
  static List<ApiError500> listFromJson(dynamic json, {bool emptyIsNull, bool growable,}) =>
    json is List && json.isNotEmpty
      ? json.map(ApiError500.fromJson).toList(growable: true == growable)
      : true == emptyIsNull ? null : <ApiError500>[];

  static Map<String, ApiError500> mapFromJson(dynamic json) {
    final map = <String, ApiError500>{};
    if (json is Map && json.isNotEmpty) {
      json
        .cast<String, dynamic>()
        .forEach((key, dynamic value) => map[key] = ApiError500.fromJson(value));
    }
    return map;
  }

  // maps a json object with a list of ApiError500-objects as value to a dart map
  static Map<String, List<ApiError500>> mapListFromJson(dynamic json, {bool emptyIsNull, bool growable,}) {
    final map = <String, List<ApiError500>>{};
    if (json is Map && json.isNotEmpty) {
      json
        .cast<String, dynamic>()
        .forEach((key, dynamic value) {
          map[key] = ApiError500.listFromJson(
            value,
